import{c,__tla as o}from"./astro-component.YcfuVWta.js";import{r as E,m as v,u as r,__tla as y}from"./constant.Cx6UCuye.js";import{__tla as x}from"./astro/assets-service.r1ocst81.js";let e,p,a,n,d,l,i,u=Promise.all([(()=>{try{return o}catch{}})(),(()=>{try{return y}catch{}})(),(()=>{try{return x}catch{}})()]).then(async()=>{let s;s=`<h2 id="function-description">Function Description</h2>
...
<p><strong>\u{1F680} Auto Protocol Compatibility</strong></p>
<p>The plugin now supports <strong>automatic protocol detection</strong>, allowing seamless compatibility with both OpenAI and Claude protocol formats without configuration:</p>
<ul>
<li><strong>OpenAI Protocol</strong>: Request path <code dir="auto">/v1/chat/completions</code>, using standard OpenAI Messages API format</li>
<li><strong>Claude Protocol</strong>: Request path <code dir="auto">/v1/messages</code>, using Anthropic Claude Messages API format</li>
<li><strong>Intelligent Conversion</strong>: Automatically detects request protocol and performs conversion if the target provider doesn\u2019t natively support it</li>
<li><strong>Zero Configuration</strong>: No need to set <code dir="auto">protocol</code> field, the plugin handles everything automatically</li>
</ul>
<blockquote>
...
<blockquote>
<p>When the request path suffix matches <code dir="auto">/v1/messages</code>, it corresponds to Claude text-to-text scenarios. The plugin automatically detects provider capabilities: if native Claude protocol is supported, requests are forwarded directly; otherwise, they are converted to OpenAI protocol first.</p>
</blockquote>
<blockquote>
...
<ol>
<li>Azure OpenAI only supports configuring one API Token.</li>
<li><code dir="auto">azureServiceUrl</code> accepts three formats\uFF1A
<ol>
<li>Full URL. e.g. <code dir="auto">https://YOUR_RESOURCE_NAME.openai.azure.com/openai/deployments/YOUR_DEPLOYMENT_NAME/chat/completions?api-version=2024-02-15-preview</code>
<ul>
<li>Request will be forwarded to the given URL, no matter what original path the request uses.</li>
</ul>
</li>
<li>Resource name + deployment name\uFF0Ce.g. <code dir="auto">https://YOUR_RESOURCE_NAME.openai.azure.com/openai/deployments/YOUR_DEPLOYMENT_NAME?api-version=2024-02-15-preview</code>
<ul>
<li>The path will be updated based on the actual request path, leaving the deployment name unchanged. APIs with no deployment name in the path are also support.</li>
</ul>
</li>
<li>Resource name only.e.g.<code dir="auto">https://YOUR_RESOURCE_NAME.openai.azure.com?api-version=2024-02-15-preview</code>
<ul>
<li>The path will be updated based on the actual request path. The deployment name will be filled based on the model name in the request and the configured model mapping rule. APIs with no deployment name in the path are also support.</li>
</ul>
</li>
</ol>
</li>
...
<h4 id="grok">Grok</h4>
<p>For Grok, the corresponding <code dir="auto">type</code> is <code dir="auto">grok</code>. It has no unique configuration fields.</p>
<h4 id="openrouter">OpenRouter</h4>
<p>For OpenRouter, the corresponding <code dir="auto">type</code> is <code dir="auto">openrouter</code>. It has no unique configuration fields.</p>
<h4 id="fireworks-ai">Fireworks AI</h4>
<p>For Fireworks AI, the corresponding <code dir="auto">type</code> is <code dir="auto">fireworks</code>. It has no unique configuration fields.</p>
...
<table><thead><tr><th>Name</th><th>Data Type</th><th>Filling Requirements</th><th>Default Value</th><th>Description</th></tr></thead><tbody><tr><td><code dir="auto">geminiSafetySetting</code></td><td>map of string</td><td>Optional</td><td>-</td><td>Gemini AI content filtering and safety level settings. Refer to <a href="https://ai.google.dev/gemini-api/docs/safety-settings" referrerpolicy="unsafe-url" rel="nofollow" target="_blank">Safety settings</a>.</td></tr><tr><td><code dir="auto">apiVersion</code></td><td>string</td><td>\u975E\u5FC5\u586B</td><td><code dir="auto">v1beta</code></td><td>To specify the version of the API, you can choose either \u2018v1\u2019 or \u2018v1beta\u2019. Version differences refer to <a href="https://ai.google.dev/gemini-api/docs/api-versions" referrerpolicy="unsafe-url" rel="nofollow" target="_blank">https://ai.google.dev/gemini-api/docs/api-versions</a></td></tr><tr><td><code dir="auto">geminiThinkingBudget</code></td><td>number</td><td>\u975E\u5FC5\u586B</td><td>-</td><td>The parameters of the gemini2.5 series: 0 indicates no thinking mode, -1 represents dynamic adjustment. For specific parameter references, please refer to the official website</td></tr></tbody></table>
...
<p>For Vertex, the corresponding <code dir="auto">type</code> is <code dir="auto">vertex</code>. Its unique configuration field is:</p>
...
<table><thead><tr><th>Name</th><th>Data Type</th><th>Requirement</th><th>Default</th><th>Description</th></tr></thead><tbody><tr><td><code dir="auto">vertexAuthKey</code></td><td>string</td><td>Required</td><td>-</td><td>Google Service Account JSON Key used for authentication. The format should be PEM encoded PKCS#8 private key along with client_email and other information</td></tr><tr><td><code dir="auto">vertexRegion</code></td><td>string</td><td>Required</td><td>-</td><td>Google Cloud region (e.g., us-central1, europe-west4) used to build the Vertex API address</td></tr><tr><td><code dir="auto">vertexProjectId</code></td><td>string</td><td>Required</td><td>-</td><td>Google Cloud Project ID, used to identify the target GCP project</td></tr><tr><td><code dir="auto">vertexAuthServiceName</code></td><td>string</td><td>Required</td><td>-</td><td>Service name for OAuth2 authentication, used to access oauth2.googleapis.com</td></tr><tr><td><code dir="auto">vertexGeminiSafetySetting</code></td><td>map of string</td><td>Optional</td><td>-</td><td>Gemini model content safety filtering settings.</td></tr><tr><td><code dir="auto">vertexTokenRefreshAhead</code></td><td>number</td><td>Optional</td><td>-</td><td>Vertex access token refresh ahead time in seconds</td></tr></tbody></table>
<h4 id="aws-bedrock">AWS Bedrock</h4>
<p>For AWS Bedrock, the corresponding <code dir="auto">type</code> is <code dir="auto">bedrock</code>. Its unique configuration field is:</p>
...
<table><thead><tr><th>Name</th><th>Data Type</th><th>Requirement</th><th>Default</th><th>Description</th></tr></thead><tbody><tr><td><code dir="auto">awsAccessKey</code></td><td>string</td><td>Required</td><td>-</td><td>AWS Access Key used for authentication</td></tr><tr><td><code dir="auto">awsSecretKey</code></td><td>string</td><td>Required</td><td>-</td><td>AWS Secret Access Key used for authentication</td></tr><tr><td><code dir="auto">awsRegion</code></td><td>string</td><td>Required</td><td>-</td><td>AWS region, e.g., us-east-1</td></tr><tr><td><code dir="auto">bedrockAdditionalFields</code></td><td>map</td><td>Optional</td><td>-</td><td>Additional inference parameters that the model supports</td></tr></tbody></table>
...
<h3 id="using-openai-protocol-proxy-for-grok-service">Using OpenAI Protocol Proxy for Grok Service</h3>
<p><strong>Configuration Information</strong></p>
...
<h3 id="using-openai-protocol-proxy-for-openrouter-service">Using OpenAI Protocol Proxy for OpenRouter Service</h3>
<p><strong>Configuration Information</strong></p>
...
<p><strong>Example Request</strong></p>
<div class="expressive-code"><figure class="frame not-content"><figcaption class="header"></figcaption><pre tabindex="0"><code><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"model"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"gpt-4"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"messages"</span><span style="--0:#E1E4E8">: [</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"role"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"user"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"content"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"Hello, who are you?"</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">  </span></span><span style="--0:#E1E4E8">],</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"temperature"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">0.7</span></div></div><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">}</span></div></div></code></pre><div class="copy"><button title="Copy to clipboard" data-copied="Copied!" data-code="{\x7F  &#x22;model&#x22;: &#x22;gpt-4&#x22;,\x7F  &#x22;messages&#x22;: [\x7F    {\x7F      &#x22;role&#x22;: &#x22;user&#x22;,\x7F      &#x22;content&#x22;: &#x22;Hello, who are you?&#x22;\x7F    }\x7F  ],\x7F  &#x22;temperature&#x22;: 0.7\x7F}"><div></div></button></div></figure></div>
<p><strong>Example Response</strong></p>
<div class="expressive-code"><figure class="frame not-content"><figcaption class="header"></figcaption><pre tabindex="0"><code><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"id"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"gen-1234567890abcdef"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"object"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"chat.completion"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"created"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">1699123456</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"model"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"openai/gpt-4-turbo-preview"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"choices"</span><span style="--0:#E1E4E8">: [</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"index"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">0</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"message"</span><span style="--0:#E1E4E8">: {</span></div></div><div class="ec-line"><div class="code"><span class="indent">        </span><span style="--0:#79B8FF">"role"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"assistant"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">        </span><span style="--0:#79B8FF">"content"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"Hello! I am an AI assistant powered by OpenRouter. I can help answer questions, assist with creative tasks, engage in conversations, and more. How can I assist you today?"</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">      </span></span><span style="--0:#E1E4E8">},</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"finish_reason"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"stop"</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">  </span></span><span style="--0:#E1E4E8">],</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"usage"</span><span style="--0:#E1E4E8">: {</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#79B8FF">"prompt_tokens"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">12</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#79B8FF">"completion_tokens"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">35</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#79B8FF">"total_tokens"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">47</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">  </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">}</span></div></div></code></pre><div class="copy"><button title="Copy to clipboard" data-copied="Copied!" data-code="{\x7F  &#x22;id&#x22;: &#x22;gen-1234567890abcdef&#x22;,\x7F  &#x22;object&#x22;: &#x22;chat.completion&#x22;,\x7F  &#x22;created&#x22;: 1699123456,\x7F  &#x22;model&#x22;: &#x22;openai/gpt-4-turbo-preview&#x22;,\x7F  &#x22;choices&#x22;: [\x7F    {\x7F      &#x22;index&#x22;: 0,\x7F      &#x22;message&#x22;: {\x7F        &#x22;role&#x22;: &#x22;assistant&#x22;,\x7F        &#x22;content&#x22;: &#x22;Hello! I am an AI assistant powered by OpenRouter. I can help answer questions, assist with creative tasks, engage in conversations, and more. How can I assist you today?&#x22;\x7F      },\x7F      &#x22;finish_reason&#x22;: &#x22;stop&#x22;\x7F    }\x7F  ],\x7F  &#x22;usage&#x22;: {\x7F    &#x22;prompt_tokens&#x22;: 12,\x7F    &#x22;completion_tokens&#x22;: 35,\x7F    &#x22;total_tokens&#x22;: 47\x7F  }\x7F}"><div></div></button></div></figure></div>
<h3 id="using-openai-protocol-proxy-for-fireworks-ai-service">Using OpenAI Protocol Proxy for Fireworks AI Service</h3>
<p><strong>Configuration Information</strong></p>
...
<p><strong>Response Example</strong></p>
<div class="expressive-code"><figure class="frame not-content"><figcaption class="header"></figcaption><pre tabindex="0"><code><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"id"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"fw-123456789"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"object"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"chat.completion"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"created"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">1699123456</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"model"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"accounts/fireworks/models/llama-v3p1-70b-instruct"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"choices"</span><span style="--0:#E1E4E8">: [</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"index"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">0</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"message"</span><span style="--0:#E1E4E8">: {</span></div></div><div class="ec-line"><div class="code"><span class="indent">        </span><span style="--0:#79B8FF">"role"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"assistant"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">        </span><span style="--0:#79B8FF">"content"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"Hello! I am an AI assistant powered by Fireworks AI, based on the Llama 3.1 model. I can help answer questions, engage in conversations, and provide various information. How can I assist you today?"</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">      </span></span><span style="--0:#E1E4E8">},</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"finish_reason"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"stop"</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">  </span></span><span style="--0:#E1E4E8">],</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"usage"</span><span style="--0:#E1E4E8">: {</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#79B8FF">"prompt_tokens"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">15</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#79B8FF">"completion_tokens"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">38</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#79B8FF">"total_tokens"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">53</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">  </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">}</span></div></div></code></pre><div class="copy"><button title="Copy to clipboard" data-copied="Copied!" data-code="{\x7F  &#x22;id&#x22;: &#x22;fw-123456789&#x22;,\x7F  &#x22;object&#x22;: &#x22;chat.completion&#x22;,\x7F  &#x22;created&#x22;: 1699123456,\x7F  &#x22;model&#x22;: &#x22;accounts/fireworks/models/llama-v3p1-70b-instruct&#x22;,\x7F  &#x22;choices&#x22;: [\x7F    {\x7F      &#x22;index&#x22;: 0,\x7F      &#x22;message&#x22;: {\x7F        &#x22;role&#x22;: &#x22;assistant&#x22;,\x7F        &#x22;content&#x22;: &#x22;Hello! I am an AI assistant powered by Fireworks AI, based on the Llama 3.1 model. I can help answer questions, engage in conversations, and provide various information. How can I assist you today?&#x22;\x7F      },\x7F      &#x22;finish_reason&#x22;: &#x22;stop&#x22;\x7F    }\x7F  ],\x7F  &#x22;usage&#x22;: {\x7F    &#x22;prompt_tokens&#x22;: 15,\x7F    &#x22;completion_tokens&#x22;: 38,\x7F    &#x22;total_tokens&#x22;: 53\x7F  }\x7F}"><div></div></button></div></figure></div>
<h3 id="using-auto-protocol-compatibility">Using Auto Protocol Compatibility</h3>
<p>The plugin now supports automatic protocol detection, capable of handling both OpenAI and Claude protocol format requests simultaneously.</p>
<p><strong>Configuration Information</strong></p>
<div class="expressive-code"><figure class="frame not-content"><figcaption class="header"></figcaption><pre tabindex="0"><code><div class="ec-line"><div class="code"><span style="--0:#85E89D">provider</span><span style="--0:#E1E4E8">:</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#85E89D">type</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">claude</span><span style="--0:#E1E4E8">  </span><span style="--0:#99A0A6"># Provider with native Claude protocol support</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#85E89D">apiTokens</span><span style="--0:#E1E4E8">:</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">- </span><span style="--0:#9ECBFF">"YOUR_CLAUDE_API_TOKEN"</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#85E89D">version</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"2023-06-01"</span></div></div></code></pre><div class="copy"><button title="Copy to clipboard" data-copied="Copied!" data-code="provider:\x7F  type: claude  # Provider with native Claude protocol support\x7F  apiTokens:\x7F    - &#x22;YOUR_CLAUDE_API_TOKEN&#x22;\x7F  version: &#x22;2023-06-01&#x22;"><div></div></button></div></figure></div>
<p><strong>OpenAI Protocol Request Example</strong></p>
...
<p><strong>Claude Protocol Request Example</strong></p>
<p>URL: <code dir="auto">http://your-domain/v1/messages</code></p>
<div class="expressive-code"><figure class="frame not-content"><figcaption class="header"></figcaption><pre tabindex="0"><code><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"model"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"claude-3-opus-20240229"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"max_tokens"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">1024</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"messages"</span><span style="--0:#E1E4E8">: [</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"role"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"user"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"content"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"Hello, who are you?"</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">  </span></span><span style="--0:#E1E4E8">]</span></div></div><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">}</span></div></div></code></pre><div class="copy"><button title="Copy to clipboard" data-copied="Copied!" data-code="{\x7F  &#x22;model&#x22;: &#x22;claude-3-opus-20240229&#x22;,\x7F  &#x22;max_tokens&#x22;: 1024,\x7F  &#x22;messages&#x22;: [\x7F    {\x7F      &#x22;role&#x22;: &#x22;user&#x22;,\x7F      &#x22;content&#x22;: &#x22;Hello, who are you?&#x22;\x7F    }\x7F  ]\x7F}"><div></div></button></div></figure></div>
<p><strong>Example Response</strong></p>
<p>Both protocol formats will return responses in their respective formats:</p>
...
<h3 id="using-intelligent-protocol-conversion">Using Intelligent Protocol Conversion</h3>
<p>When the target provider doesn\u2019t natively support Claude protocol, the plugin automatically performs protocol conversion:</p>
<p><strong>Configuration Information</strong></p>
<div class="expressive-code"><figure class="frame not-content"><figcaption class="header"></figcaption><pre tabindex="0"><code><div class="ec-line"><div class="code"><span style="--0:#85E89D">provider</span><span style="--0:#E1E4E8">:</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#85E89D">type</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">qwen</span><span style="--0:#E1E4E8">  </span><span style="--0:#99A0A6"># Doesn't natively support Claude protocol, auto-conversion applied</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#85E89D">apiTokens</span><span style="--0:#E1E4E8">:</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">- </span><span style="--0:#9ECBFF">"YOUR_QWEN_API_TOKEN"</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#85E89D">modelMapping</span><span style="--0:#E1E4E8">:</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#9ECBFF">'claude-3-opus-20240229'</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">'qwen-max'</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#9ECBFF">'*'</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">'qwen-turbo'</span></div></div></code></pre><div class="copy"><button title="Copy to clipboard" data-copied="Copied!" data-code="provider:\x7F  type: qwen  # Doesn&#x27;t natively support Claude protocol, auto-conversion applied\x7F  apiTokens:\x7F    - &#x22;YOUR_QWEN_API_TOKEN&#x22;\x7F  modelMapping:\x7F    &#x27;claude-3-opus-20240229&#x27;: &#x27;qwen-max&#x27;\x7F    &#x27;*&#x27;: &#x27;qwen-turbo&#x27;"><div></div></button></div></figure></div>
<p><strong>Claude Protocol Request</strong></p>
<p>URL: <code dir="auto">http://your-domain/v1/messages</code> (automatically converted to OpenAI protocol for provider)</p>
<div class="expressive-code"><figure class="frame not-content"><figcaption class="header"></figcaption><pre tabindex="0"><code><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"model"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"claude-3-opus-20240229"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"max_tokens"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">1024</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"messages"</span><span style="--0:#E1E4E8">: [</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"role"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"user"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"content"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"Hello, who are you?"</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">  </span></span><span style="--0:#E1E4E8">]</span></div></div><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">}</span></div></div></code></pre><div class="copy"><button title="Copy to clipboard" data-copied="Copied!" data-code="{\x7F  &#x22;model&#x22;: &#x22;claude-3-opus-20240229&#x22;,\x7F  &#x22;max_tokens&#x22;: 1024,\x7F  &#x22;messages&#x22;: [\x7F    {\x7F      &#x22;role&#x22;: &#x22;user&#x22;,\x7F      &#x22;content&#x22;: &#x22;Hello, who are you?&#x22;\x7F    }\x7F  ]\x7F}"><div></div></button></div></figure></div>
...
<h3 id="utilizing-openai-protocol-proxy-for-google-vertex-services">Utilizing OpenAI Protocol Proxy for Google Vertex Services</h3>
<p><strong>Configuration Information</strong></p>
...
<p><strong>Request Example</strong></p>
<div class="expressive-code"><figure class="frame not-content"><figcaption class="header"></figcaption><pre tabindex="0"><code><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"model"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"gemini-2.0-flash-001"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"messages"</span><span style="--0:#E1E4E8">: [</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"role"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"user"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"content"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"Who are you?"</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">  </span></span><span style="--0:#E1E4E8">],</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"stream"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">false</span></div></div><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">}</span></div></div></code></pre><div class="copy"><button title="Copy to clipboard" data-copied="Copied!" data-code="{\x7F  &#x22;model&#x22;: &#x22;gemini-2.0-flash-001&#x22;,\x7F  &#x22;messages&#x22;: [\x7F    {\x7F      &#x22;role&#x22;: &#x22;user&#x22;,\x7F      &#x22;content&#x22;: &#x22;Who are you?&#x22;\x7F    }\x7F  ],\x7F  &#x22;stream&#x22;: false\x7F}"><div></div></button></div></figure></div>
<p><strong>Response Example</strong></p>
<div class="expressive-code"><figure class="frame not-content"><figcaption class="header"></figcaption><pre tabindex="0"><code><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"id"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"chatcmpl-0000000000000"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"choices"</span><span style="--0:#E1E4E8">: [</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"index"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">0</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"message"</span><span style="--0:#E1E4E8">: {</span></div></div><div class="ec-line"><div class="code"><span class="indent">        </span><span style="--0:#79B8FF">"role"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"assistant"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">        </span><span style="--0:#79B8FF">"content"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"Hello! I am the Gemini model provided by Vertex AI, developed by Google. I can answer questions, provide information, and assist in completing various tasks. How can I help you today?"</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">      </span></span><span style="--0:#E1E4E8">},</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"finish_reason"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"stop"</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">  </span></span><span style="--0:#E1E4E8">],</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"created"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">1729986750</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"model"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"gemini-2.0-flash-001"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"object"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"chat.completion"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"usage"</span><span style="--0:#E1E4E8">: {</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#79B8FF">"prompt_tokens"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">15</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#79B8FF">"completion_tokens"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">43</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#79B8FF">"total_tokens"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">58</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">  </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">}</span></div></div></code></pre><div class="copy"><button title="Copy to clipboard" data-copied="Copied!" data-code="{\x7F  &#x22;id&#x22;: &#x22;chatcmpl-0000000000000&#x22;,\x7F  &#x22;choices&#x22;: [\x7F    {\x7F      &#x22;index&#x22;: 0,\x7F      &#x22;message&#x22;: {\x7F        &#x22;role&#x22;: &#x22;assistant&#x22;,\x7F        &#x22;content&#x22;: &#x22;Hello! I am the Gemini model provided by Vertex AI, developed by Google. I can answer questions, provide information, and assist in completing various tasks. How can I help you today?&#x22;\x7F      },\x7F      &#x22;finish_reason&#x22;: &#x22;stop&#x22;\x7F    }\x7F  ],\x7F  &#x22;created&#x22;: 1729986750,\x7F  &#x22;model&#x22;: &#x22;gemini-2.0-flash-001&#x22;,\x7F  &#x22;object&#x22;: &#x22;chat.completion&#x22;,\x7F  &#x22;usage&#x22;: {\x7F    &#x22;prompt_tokens&#x22;: 15,\x7F    &#x22;completion_tokens&#x22;: 43,\x7F    &#x22;total_tokens&#x22;: 58\x7F  }\x7F}"><div></div></button></div></figure></div>
<h3 id="utilizing-openai-protocol-proxy-for-aws-bedrock-services">Utilizing OpenAI Protocol Proxy for AWS Bedrock Services</h3>
<p><strong>Configuration Information</strong></p>
...
<p><strong>Request Example</strong></p>
<div class="expressive-code"><figure class="frame not-content"><figcaption class="header"></figcaption><pre tabindex="0"><code><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"model"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"arn:aws:bedrock:us-west-2::foundation-model/anthropic.claude-3-5-haiku-20241022-v1:0"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"messages"</span><span style="--0:#E1E4E8">: [</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"role"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"user"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"content"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"who are you"</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">  </span></span><span style="--0:#E1E4E8">],</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"stream"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">false</span></div></div><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">}</span></div></div></code></pre><div class="copy"><button title="Copy to clipboard" data-copied="Copied!" data-code="{\x7F  &#x22;model&#x22;: &#x22;arn:aws:bedrock:us-west-2::foundation-model/anthropic.claude-3-5-haiku-20241022-v1:0&#x22;,\x7F  &#x22;messages&#x22;: [\x7F    {\x7F      &#x22;role&#x22;: &#x22;user&#x22;,\x7F      &#x22;content&#x22;: &#x22;who are you&#x22;\x7F    }\x7F  ],\x7F  &#x22;stream&#x22;: false\x7F}"><div></div></button></div></figure></div>
<p><strong>Response Example</strong></p>
<div class="expressive-code"><figure class="frame not-content"><figcaption class="header"></figcaption><pre tabindex="0"><code><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"id"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"d52da49d-daf3-49d9-a105-0b527481fe14"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"choices"</span><span style="--0:#E1E4E8">: [</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"index"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">0</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"message"</span><span style="--0:#E1E4E8">: {</span></div></div><div class="ec-line"><div class="code"><span class="indent">        </span><span style="--0:#79B8FF">"role"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"assistant"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">        </span><span style="--0:#79B8FF">"content"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"I'm Claude, an AI created by Anthropic. I aim to be helpful, honest, and harmless. I won't pretend to be human, and I'll always try to be direct and truthful about what I am and what I can do."</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">      </span></span><span style="--0:#E1E4E8">},</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"finish_reason"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"stop"</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">  </span></span><span style="--0:#E1E4E8">],</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"created"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">1749659050</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"model"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"arn:aws:bedrock:us-west-2::foundation-model/anthropic.claude-3-5-haiku-20241022-v1:0"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"object"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"chat.completion"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"usage"</span><span style="--0:#E1E4E8">: {</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#79B8FF">"prompt_tokens"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">10</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#79B8FF">"completion_tokens"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">57</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#79B8FF">"total_tokens"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">67</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">  </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">}</span></div></div></code></pre><div class="copy"><button title="Copy to clipboard" data-copied="Copied!" data-code="{\x7F  &#x22;id&#x22;: &#x22;d52da49d-daf3-49d9-a105-0b527481fe14&#x22;,\x7F  &#x22;choices&#x22;: [\x7F    {\x7F      &#x22;index&#x22;: 0,\x7F      &#x22;message&#x22;: {\x7F        &#x22;role&#x22;: &#x22;assistant&#x22;,\x7F        &#x22;content&#x22;: &#x22;I&#x27;m Claude, an AI created by Anthropic. I aim to be helpful, honest, and harmless. I won&#x27;t pretend to be human, and I&#x27;ll always try to be direct and truthful about what I am and what I can do.&#x22;\x7F      },\x7F      &#x22;finish_reason&#x22;: &#x22;stop&#x22;\x7F    }\x7F  ],\x7F  &#x22;created&#x22;: 1749659050,\x7F  &#x22;model&#x22;: &#x22;arn:aws:bedrock:us-west-2::foundation-model/anthropic.claude-3-5-haiku-20241022-v1:0&#x22;,\x7F  &#x22;object&#x22;: &#x22;chat.completion&#x22;,\x7F  &#x22;usage&#x22;: {\x7F    &#x22;prompt_tokens&#x22;: 10,\x7F    &#x22;completion_tokens&#x22;: 57,\x7F    &#x22;total_tokens&#x22;: 67\x7F  }\x7F}"><div></div></button></div></figure></div>
<h3 id="utilizing-openai-protocol-proxy-for-nvidia-triton-interference-server-services">Utilizing OpenAI Protocol Proxy for NVIDIA Triton Interference Server Services</h3>
<p><strong>Configuration Information</strong></p>
...
<p><strong>Request Example</strong></p>
<div class="expressive-code"><figure class="frame not-content"><figcaption class="header"></figcaption><pre tabindex="0"><code><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"model"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"gpt2"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"messages"</span><span style="--0:#E1E4E8">: [</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"role"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"user"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">      </span><span style="--0:#79B8FF">"content"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"Hi, who are you\uFF1F"</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">  </span></span><span style="--0:#E1E4E8">],</span></div></div><div class="ec-line"><div class="code"><span class="indent">  </span><span style="--0:#79B8FF">"stream"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">false</span></div></div><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">}</span></div></div></code></pre><div class="copy"><button title="Copy to clipboard" data-copied="Copied!" data-code="{\x7F  &#x22;model&#x22;: &#x22;gpt2&#x22;,\x7F  &#x22;messages&#x22;: [\x7F    {\x7F      &#x22;role&#x22;: &#x22;user&#x22;,\x7F      &#x22;content&#x22;: &#x22;Hi, who are you\uFF1F&#x22;\x7F    }\x7F  ],\x7F  &#x22;stream&#x22;: false\x7F}"><div></div></button></div></figure></div>
<p><strong>Response Example</strong></p>
<div class="expressive-code"><figure class="frame not-content"><figcaption class="header"></figcaption><pre tabindex="0"><code><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#79B8FF">"choices"</span><span style="--0:#E1E4E8">: [</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">        </span></span><span style="--0:#E1E4E8">{</span></div></div><div class="ec-line"><div class="code"><span class="indent">            </span><span style="--0:#79B8FF">"index"</span><span style="--0:#E1E4E8">: </span><span style="--0:#79B8FF">0</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">            </span><span style="--0:#79B8FF">"message"</span><span style="--0:#E1E4E8">: {</span></div></div><div class="ec-line"><div class="code"><span class="indent">                </span><span style="--0:#79B8FF">"role"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"assistant"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent">                </span><span style="--0:#79B8FF">"content"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"I am a lagguage model."</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">            </span></span><span style="--0:#E1E4E8">},</span></div></div><div class="ec-line"><div class="code"><span class="indent">            </span><span style="--0:#79B8FF">"finish_reason"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"stop"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">        </span></span><span style="--0:#E1E4E8">}</span></div></div><div class="ec-line"><div class="code"><span class="indent"><span style="--0:#E1E4E8">    </span></span><span style="--0:#E1E4E8">],</span></div></div><div class="ec-line"><div class="code"><span class="indent">    </span><span style="--0:#79B8FF">"model"</span><span style="--0:#E1E4E8">: </span><span style="--0:#9ECBFF">"gpt2"</span><span style="--0:#E1E4E8">,</span></div></div><div class="ec-line"><div class="code"><span style="--0:#E1E4E8">}</span></div></div></code></pre><div class="copy"><button title="Copy to clipboard" data-copied="Copied!" data-code="{\x7F    &#x22;choices&#x22;: [\x7F        {\x7F            &#x22;index&#x22;: 0,\x7F            &#x22;message&#x22;: {\x7F                &#x22;role&#x22;: &#x22;assistant&#x22;,\x7F                &#x22;content&#x22;: &#x22;I am a lagguage model.&#x22;\x7F            },\x7F            &#x22;finish_reason&#x22;: &#x22;stop&#x22;,\x7F        }\x7F    ],\x7F    &#x22;model&#x22;: &#x22;gpt2&#x22;,\x7F}"><div></div></button></div></figure></div>
...
**\u{1F680} Auto Protocol Compatibility**
...
\`\`\`
`},p=function(){return s},d=function(){return[{depth:2,slug:"function-description",text:"Function Description"},{depth:2,slug:"execution-properties",text:"Execution Properties"},{depth:2,slug:"configuration-fields",text:"Configuration Fields"},{depth:3,slug:"basic-configuration",text:"Basic Configuration"},{depth:3,slug:"provider-specific-configurations",text:"Provider-Specific Configurations"},{depth:4,slug:"openai",text:"OpenAI"},{depth:4,slug:"azure-openai",text:"Azure OpenAI"},{depth:4,slug:"moonshot",text:"Moonshot"},{depth:4,slug:"qwen-tongyi-qwen",text:"Qwen (Tongyi Qwen)"},{depth:4,slug:"baichuan-ai",text:"Baichuan AI"},{depth:4,slug:"yi-zero-one-universe",text:"Yi (Zero One Universe)"},{depth:4,slug:"zhipu-ai",text:"Zhipu AI"},{depth:4,slug:"deepseek",text:"DeepSeek"},{depth:4,slug:"groq",text:"Groq"},{depth:4,slug:"grok",text:"Grok"},{depth:4,slug:"openrouter",text:"OpenRouter"},{depth:4,slug:"fireworks-ai",text:"Fireworks AI"},{depth:4,slug:"ernie-bot",text:"ERNIE Bot"},{depth:3,slug:"360-brain",text:"360 Brain"},{depth:3,slug:"mistral",text:"Mistral"},{depth:4,slug:"minimax",text:"MiniMax"},{depth:4,slug:"anthropic-claude",text:"Anthropic Claude"},{depth:4,slug:"ollama",text:"Ollama"},{depth:4,slug:"hunyuan",text:"Hunyuan"},{depth:4,slug:"stepfun",text:"Stepfun"},{depth:4,slug:"cloudflare-workers-ai",text:"Cloudflare Workers AI"},{depth:4,slug:"spark",text:"Spark"},{depth:4,slug:"gemini",text:"Gemini"},{depth:3,slug:"deepl",text:"DeepL"},{depth:4,slug:"google-vertex-ai",text:"Google Vertex AI"},{depth:4,slug:"aws-bedrock",text:"AWS Bedrock"},{depth:2,slug:"usage-examples",text:"Usage Examples"},{depth:3,slug:"using-openai-protocol-proxy-for-azure-openai-service",text:"Using OpenAI Protocol Proxy for Azure OpenAI Service"},{depth:3,slug:"using-openai-protocol-proxy-for-qwen-service",text:"Using OpenAI Protocol Proxy for Qwen Service"},{depth:3,slug:"using-qwen-service-with-pure-text-context-information",text:"Using Qwen Service with Pure Text Context Information"},{depth:3,slug:"using-qwen-service-with-native-file-context",text:"Using Qwen Service with Native File Context"},{depth:3,slug:"forwards-requests-to-alicloud-bailian-with-the-original-protocol",text:"Forwards requests to AliCloud Bailian with the \u201Coriginal\u201D protocol"},{depth:3,slug:"using-openai-protocol-proxy-for-doubao-service",text:"Using OpenAI Protocol Proxy for Doubao Service"},{depth:3,slug:"using-original-protocol-proxy-for-coze-applications",text:"Using original Protocol Proxy for Coze applications"},{depth:3,slug:"utilizing-moonshot-with-its-native-file-context",text:"Utilizing Moonshot with its Native File Context"},{depth:3,slug:"using-openai-protocol-proxy-for-groq-service",text:"Using OpenAI Protocol Proxy for Groq Service"},{depth:3,slug:"using-openai-protocol-proxy-for-grok-service",text:"Using OpenAI Protocol Proxy for Grok Service"},{depth:3,slug:"using-openai-protocol-proxy-for-openrouter-service",text:"Using OpenAI Protocol Proxy for OpenRouter Service"},{depth:3,slug:"using-openai-protocol-proxy-for-fireworks-ai-service",text:"Using OpenAI Protocol Proxy for Fireworks AI Service"},{depth:3,slug:"using-auto-protocol-compatibility",text:"Using Auto Protocol Compatibility"},{depth:3,slug:"using-intelligent-protocol-conversion",text:"Using Intelligent Protocol Conversion"},{depth:3,slug:"using-openai-protocol-proxy-for-hunyuan-service",text:"Using OpenAI Protocol Proxy for Hunyuan Service"},{depth:3,slug:"using-openai-protocol-proxy-for-ernie-bot-service",text:"Using OpenAI Protocol Proxy for ERNIE Bot Service"},{depth:3,slug:"using-openai-protocol-proxy-for-minimax-service",text:"Using OpenAI Protocol Proxy for MiniMax Service"},{depth:3,slug:"using-openai-protocol-proxy-for-360-brain-services",text:"Using OpenAI Protocol Proxy for 360 Brain Services"},{depth:3,slug:"using-openai-protocol-proxy-for-cloudflare-workers-ai-service",text:"Using OpenAI Protocol Proxy for Cloudflare Workers AI Service"},{depth:3,slug:"using-openai-protocol-proxy-for-spark-service",text:"Using OpenAI Protocol Proxy for Spark Service"},{depth:3,slug:"utilizing-openai-protocol-proxy-for-gemini-services",text:"Utilizing OpenAI Protocol Proxy for Gemini Services"},{depth:3,slug:"utilizing-openai-protocol-proxy-for-deepl-text-translation-service",text:"Utilizing OpenAI Protocol Proxy for DeepL Text Translation Service"},{depth:3,slug:"utilizing-openai-protocol-proxy-for-together-ai-services",text:"Utilizing OpenAI Protocol Proxy for Together-AI Services"},{depth:3,slug:"utilizing-openai-protocol-proxy-for-google-vertex-services",text:"Utilizing OpenAI Protocol Proxy for Google Vertex Services"},{depth:3,slug:"utilizing-openai-protocol-proxy-for-aws-bedrock-services",text:"Utilizing OpenAI Protocol Proxy for AWS Bedrock Services"},{depth:3,slug:"utilizing-openai-protocol-proxy-for-nvidia-triton-interference-server-services",text:"Utilizing OpenAI Protocol Proxy for NVIDIA Triton Interference Server Services"},{depth:2,slug:"full-configuration-example",text:"Full Configuration Example"},{depth:3,slug:"kubernetes-example",text:"Kubernetes Example"},{depth:3,slug:"docker-compose-example",text:"Docker-Compose Example"}]},e=c((f,F,g)=>{const{layout:m,...t}=n;return t.file=a,t.url=i,E`${v()}${r(s)}`})});export{e as Content,u as __tla,p as compiledContent,e as default,a as file,n as frontmatter,d as getHeadings,l as rawContent,i as url};
